...
As we can see, the two models are not significantly different, so the effect of the grouping variable is not significant.  Since the grouping variable introduced when combining th C14 and OSL ages into a single data set represents the respective datasets, the polynomial fits to the two datasets can be considered not significantly different.
