...
```{r}
# We need to cobone OSL and C14, with a col for square, a col for depth, a col for sample ID, and a col for approx age
...
osl_ages_rugplot_data$id <- ifelse(osl_ages_rugplot_data$id == "NE1C", 
                                   "NE1B", osl_ages_rugplot_data$id)
...
svg("figures/Artefact density by depth in squares B-C 4-5-6.svg",
    width = 10, height = 7)
gt = ggplot_gtable(ggplot_build(p))
gt$layout$clip = "off"
grid.draw(gt)
...
ages_with_groups <- data.frame(ages = c(osl_ages_SWA$osl_age,
                               ...
                               depths = c(osl_ages_SWA$total_station_depth_below_surf,
                                          osl_ages_SWB$total_station_depth_below_surf),
