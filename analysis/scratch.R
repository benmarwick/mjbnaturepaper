...
# I made some edits to this sheet to make the GS numbers more consistent, and move details
# added by EH to the codes into a 'notes' column
...
# Desc fragment to match with in the ts data
gs_phases_sep <-
  gs_phases %>%
  separate(`Spit/Square`, c("square", "spit"), "/") %>%
  mutate(square_spit_gs = paste0(square, "_", spit, "_", sub("\\s", "", gs_phases$`Artefact no.`)))

# join total station data to Ebbe's data, at this point, for some of Ebbe's artefacts we have
# multiple rows because some artefacts had multiple points shot on them. We'll keep them for now
join_total_station_data_to_Ebbes_data <-
  cleaned_rotated_points_in_main_excavation_area %>%  # ts points
  mutate(Description_short = gsub("[A-Z]{1,2}_", "", cleaned_rotated_points_in_main_excavation_area$Description)) %>%
  inner_join(gs_phases_sep,                            # Ebbe's data
...
# what are we missing? Which of Ebbe's Artefact no. are missing from our table with phases?
missing_artefact_no <- gs_phases$`Artefact no.`[!(gs_phases$`Artefact no.` %in% ebbes_artefacts_with_phases$`Artefact no.`)]

# probably we have a mismatch between the spit-square data in Ebbe's sheet and the total station data. Let's look into that, and get phases for these based on artefact number only, excluding Ebbe's sq-spit data

# prep a colum of GS# or L# or R# for joining
library(stringr)
cleaned_rotated_points_in_main_excavation_area$GS_number <-
  str_extract(cleaned_rotated_points_in_main_excavation_area$Description,
              "(GS|L|R)(\\d{1,3}$)")

gs_phases_missing <-
  gs_phases_sep %>%
  mutate(Ebbes_artefact_number = sub("\\s", "", `Artefact no.`)) %>%
  filter(`Artefact no.` %in% missing_artefact_no) %>%
  inner_join(cleaned_rotated_points_in_main_excavation_area,
             by = c('Ebbes_artefact_number' = 'GS_number')) %>%
  select(Site            ,          `Artefact no.`            ,
          `year collected`,        `Artefact type`            ,
          square.x       ,          spit.x                    ,
          `PHASE #`      ,          Notes                     ,
          square_spit_gs ,          Ebbes_artefact_number     ,
...
# Compute difference in spits between
gs_phases_missing_and_diffs <-
gs_phases_missing %>%
  mutate(Description = gsub("\\s", "", Description)) %>%
  filter(!grepl("SR", Description )) %>%  # don't want these to match with R
  mutate(diff_in_spits =
         abs(as.numeric(gsub("[A-Z]{1,2}$", "", spit.x)) -
             as.numeric(gsub("(.{1,2}_.{2}_)|[A-Z]|{1}_(GS|L|R)(\\d{1,3}$)", "", Description))  ))  %>%
  rename(Ebbes_square_spit_data = square_spit_gs) %>%
  rename(Total_station_square_spit_data = Description) %>%
  group_by(Ebbes_square_spit_data,
           Total_station_square_spit_data,
           diff_in_spits) %>%
  slice(1) %>%
  select(Ebbes_square_spit_data,
         Total_station_square_spit_data,
         diff_in_spits,
         depth_below_ground_surface) %>%
  arrange(desc(diff_in_spits))

# add phases on based on the total station depths.
ebbes_missing_artefacts_with_phases <-
  fuzzy_left_join(data.frame(gs_phases_missing_and_diffs$depth_below_ground_surface),
    phases,
    by = c("gs_phases_missing_and_diffs.depth_below_ground_surface" = "upper",
           "gs_phases_missing_and_diffs.depth_below_ground_surface" = "lower"),
            match_fun = list(`>=`, `<=`)) %>%
  distinct() %>%
  left_join(gs_phases_missing_and_diffs,
            by = c("gs_phases_missing_and_diffs.depth_below_ground_surface" =
                     "depth_below_ground_surface")) %>%
  select(-upper, -lower) %>%
  rename(depth_below_ground_surface =
         gs_phases_missing_and_diffs.depth_below_ground_surface)

write.csv(ebbes_missing_artefacts_with_phases,
  "C:/Users/bmarwick/Desktop/Ebbes_artefacts_with_square_spit_mismatches.csv")
...
# also want to plot
# also want to adjust for slope
# also for the mismatches, find the square and spit of the artefact based on nearby EL
...
six_artefacts <-
ebbes_artefacts_with_phases %>%
  filter(grepl(paste0(six, collapse = "|"), Description))

library(ggplot2)
library(ggrepel)
ggplot(six_artefacts,
       aes(Xnew_flipped,
           ts_depths)) +
  scale_y_reverse() +
  ylim(3,0) +
...
# plot just these six with all the flakes, etc.
...
mutate(spit_square = paste0(square, "_", spit))
...
missing_sq_sp <- gs_deps_phases_dets[is.na(gs_deps_phases_dets$phase), ]$square_spit.x

# find ave depth for aretfacts in the sq-sp that we dont have the  gs data for
grindind_stones_missing_sq_sp <-
  stone_artefacts_only %>%
  filter(grepl(paste0(missing_sq_sp, collapse ="|"), Description)) %>%
...
# assign phases to these
gs_deps_missing_phases <-
  fuzzy_left_join(
    grindind_stones_missing_sq_sp,
    phases,
    by = c(
      "mean_depth_below_ground_surface" = "upper",
      "mean_depth_below_ground_surface" = "lower"
    ),
    match_fun = list(`>=`, `<=`)
  ) %>%
  group_by(mean_depth_below_ground_surface) %>%
  slice(1)

# join back into Ebbe's data
gs_deps_phases_dets_missing_sq_sp <-
  gs_deps_phases_dets %>%
  left_join(gs_deps_missing_phases,
            by = c('square_spit.x' = 'square_spit'))

# combine what we know about phases into 1 col
gs_deps_phases_dets_missing_sq_sp$`PHASE #` <-
  ifelse(!is.na(gs_deps_phases_dets_missing_sq_sp$phase.x),
         gs_deps_phases_dets_missing_sq_sp$phase.x,
         gs_deps_phases_dets_missing_sq_sp$phase.y)

# what are we missing now?
gs_deps_phases_dets_missing_sq_sp %>%
  filter(is.na(`PHASE #`)) %>%
  select(square_spit.x)

#   C3_44 # only end levels
#   NA_NA

# check total station data
c3_44 <-
cleaned_rotated_points_in_main_excavation_area %>%
  filter(grepl("C3_44", Description))

c3_44$square_spit <- with(c3_44, paste0(square, "_", spit))
c3_44_mean_depth <- mean(c3_44$depth_below_ground_surface)

# get c3_44 depths from end levels
gs_deps_phases_dets_missing_sq_sp <-
gs_deps_phases_dets_missing_sq_sp %>%
  mutate(mean_depth_below_ground_surface.x = ifelse(square_spit.x == "C3_44", c3_44_mean_depth, mean_depth_below_ground_surface.x))

# get phase for this depth of c3-44
row_of_c3_44_with_phase <-
  fuzzy_left_join(
    gs_deps_phases_dets_missing_sq_sp[gs_deps_phases_dets_missing_sq_sp$square_spit.x ==  "C3_44", ],
    phases,
    by = c(
      "mean_depth_below_ground_surface.x" = "upper",
      "mean_depth_below_ground_surface.x" = "lower"
    ),
    match_fun = list(`>=`, `<=`)
  ) %>%
  group_by(mean_depth_below_ground_surface.x) %>%
  slice(1)

# update the table by replacing the phase value
gs_deps_phases_dets_missing_sq_sp %>%
  mutate(`PHASE #` = ifelse(square_spit.x == "C3_44", row_of_c3_44_with_phase$phase, `PHASE #`))

# now what are we missing?
gs_deps_phases_dets_missing_sq_sp[is.na(gs_deps_phases_dets_missing_sq_sp$`PHASE #`), ]$`Artefact no.`

# [1] "UP GS 36" "GS 85"

# search in all points
points_in_main_excavation_area %>%
  filter(grepl("GS29", Description))

stone_artefacts_only %>%
  filter(grepl("GS53", Description))

# check sq-sp mastching based on GS number & Ebbe's data

# This is based on matching by GS number, let's see what the difference is
# between what Ebbe has, and what the TS has in square_spit
gs_diffs_in_square_spit_EH_v_total_station <-
  gs_deps_phases_dets_missing_sq_sp %>%
  mutate(check_square_spits = ifelse(square_spit.x == square_spit.y,
                                     "same", "different")) %>%
  select(`PHASE #` ,
         `Artefact no.`,
         square_spit.x,
         ...
         check_square_spits) %>%
  mutate(diff_in_spits =
    abs(as.numeric(gsub(".*_|[A-Z]{1,2}$", "", gs_deps_phases_dets_missing_sq_sp$square_spit.x)) -
        as.numeric(gsub(".*_|[A-Z]{1,2}$", "", gs_deps_phases_dets_missing_sq_sp$square_spit.y))
        ))  %>%
  rename(Ebbes_square_spit_data = square_spit.x) %>%
  rename(Total_station_square_spit_data = square_spit.y) %>%
  arrange(desc(diff_in_spits), check_square_spits)
# quite a few not the same, based on the matching by GS#
# let's just focus on GS where there are several spits difference, since
# that could effect the phase that it should go in.
write.csv(gs_diffs_in_square_spit_EH_v_total_station,
          "gs_diffs_in_square_spit_EH_v_total_station.csv")

# get all the spit numbers, minus square and "A" suffixes
# gsub substitute
# .*_   everything before _, eg B3_
# |  or, as in 'this or that'
# [A-Z]{1,2}$ any 1 or two capital letters at the end of the string
# "" substitute that with nothing (ie. delete)


# checking a few in case there were multiple GS-spit matches
points_in_main_excavation_area %>%
  filter(grepl("GS53", Description))
# nothing at all
